...
def upd_run_baseline_models_cv(df, drug, label_col, record_time=True, random_state=2025, n_splits=5):
    ...
    gene_cols = [
        ...
        if col not in [
            ...
            'DATASET', 'NLME_RESULT_ID', 'NLME_CURVE_ID', 'SANGER_MODEL_ID','PUTATIVE_TARGET',
            'PATHWAY_NAME', 'COMPANY_ID', 'WEBRELEASE', 'MIN_CONC', 'MAX_CONC', 'AUC', 'RMSE', 'Z_SCORE',
            'SAMPLE_NAME', 'GDSC_TD1', 'GDSC_TD2','MSI'
        ]
    ]
    
    # Including these variables as X: Gene expression columns
    # 'WES', 'CNA', 'GENE_EXPRESSION', 'Methylation', 'DRUG_RESPONSE', 'TCGA_DESC', 'CANCER_TYPE', 'SCREEN_MEDIUM', 'GROWTH_PROPERTIES'
